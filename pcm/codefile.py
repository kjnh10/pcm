...
class CodeFile(object):
    ...
    def submit(self, config: Config, language: str) -> None:
        ...
        code_string = self.bundle(config, expand_acl=(False if (contest_site == 'AtCoder' and lang_id in ['5028', '5031']) else True)).encode()
    ...
    def bundle(self, config: Config, expand_acl: bool = True) -> str:
        if self.extension == "cpp":
            ...
            if expand_acl:
                ...
                exist_include_acl = False
                with open(self.path, mode='r') as f:
                    lines = f.read()
                    if re.search(r'^#include .atcoder/.*', lines):
                        exist_include_acl = True
